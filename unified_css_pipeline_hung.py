...
class AdvancedHybridMatcher:
    ...
    def match(self, est_sources, asd_scores, chunk_idx):
        ...
        # 🚀 [Optimization] 임베딩 중복 추출 제거 (추론 시간 단축)
        channel_embeddings = []
        for k in range(K):
        ...
        # -------------------------------------------------------------
        # 🎯 STEP 1: 정교한 매칭 점수판(Cost Matrix) 구성 (Fix 1 & Fix 2 적용)
        # -------------------------------------------------------------
        for n in range(N):
            ...
            for k in range(K):
                ...
                # [Fix 1] AV Correlation 불안정성 방어 (기본 점수 0.5 보장)
                if is_face_tracked:
                ...
                if vis_prob > 0.8:
                    ...
                    # [Fix 2] 화면에 있는데 입을 다물면, 오디오가 아무리 비슷해도 LONG 점수 1/10 토막 페널티
        ...
        # -------------------------------------------------------------
        # 🎯 STEP 2: 매칭 대상 결정 로직 (Fix 3: Vision-First Greedy)
        # -------------------------------------------------------------
        active_faces = [n for n in range(N) if max_vis_scores[n].item() > 0.8]
        row_ind, col_ind = [], []
        
        # 확실하게 발화 중인 사람이 1명뿐일 때 -> 독점 선점권(Greedy) 부여
        if len(active_faces) == 1:
            ...
            # 동시 발화(2명 이상)이거나 전원 침묵일 때 -> 전체 최적화(헝가리안)
            row_ind, col_ind = linear_sum_assignment(-S_total.numpy())
        
        # -------------------------------------------------------------
        # 🎯 STEP 3: 채널 할당 및 Golden Feature 안전 업데이트
        # -------------------------------------------------------------
        ...
        for r, c in zip(row_ind, col_ind):
            ...
            # Rejection: 점수가 기준 미달(0.15)이면 버림
            ...
            # 버퍼 업데이트: 미리 뽑아둔 임베딩(channel_embeddings[r]) 재사용
