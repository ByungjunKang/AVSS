...
def process_single_video(video_path, output_npy_path, DET, ASD_model):
    ...
    try:
        ...
        # 3. 메인 화자(Target Speaker) 선택: 길이가 가장 긴 트랙
        # VoxCeleb2는 대부분 메인 화자 1명이므로 모두 Crop할 필요 없음
        main_track = max(allTracks, key=lambda x: len(x['frame']))
        
        # 4. 선택된 트랙만 Crop 및 추론
        crop_file_path = os.path.join(args.pycropPath, 'target_speaker')
        crop_video(args, main_track, crop_file_path)
        ...
        numpy.save(output_npy_path, scores)
        
        return True
        
    finally:
        # 7. 디스크 용량 관리를 위해 임시 폴더 즉각 삭제
