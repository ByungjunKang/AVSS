...
class Recurrent(nn.Module):
    def __init__(
        ...
        # 🚀 [수정 1] 이전 청크의 기억을 유지할 가벼운 Band-wise GRU 추가
        # out_channels(128) 차원의 피처를 받아 동일한 차원으로 뱉습니다.
        self.context_rnn = nn.GRU(out_channels, out_channels, num_layers=1, batch_first=True)
    ...
    # 🚀 [수정 2] state 인자 추가 및 GRU 통과 로직 삽입
    def forward(self, x, state=None):
        # x shape: B, nband, N, T (N은 out_channels=128)
        B, nband, N, T = x.shape
        
        # 1. GRU 연산을 위해 형태 변경 -> [B * nband, T, N]
        # 각 주파수 대역(band)이 독립적인 배치(Batch)처럼 작동하게 합니다.
        x_rnn = x.permute(0, 1, 3, 2).contiguous().view(B * nband, T, N)
        
        # 2. GRU에 현재 피처와 이전 청크의 state를 통과
        x_rnn, new_state = self.context_rnn(x_rnn, state)
        
        # 3. 다시 원래 형태로 복구 -> [B, nband, N, T]
        x = x_rnn.view(B, nband, T, N).permute(0, 1, 3, 2).contiguous()

        # 4. 기존 Attention + Conv 반복 로직 (그대로 유지)
        ...
        # 🚀 [수정 3] 결과물과 함께 new_state 반환
        return x.permute(0, 2, 1, 3).contiguous(), new_state
...
class TIGER(BaseModel):
    ...
    def forward(self, input, state=None):
        ...
        # 🚀 [수정 5] separator에 state 전달 및 반환받기
        sep_output, new_state = self.separator(subband_feature.view(batch_size*nch, self.nband, self.feature_dim, -1), state=state)  
